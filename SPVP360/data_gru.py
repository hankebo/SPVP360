...
class VRVideo(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.train:
            return   img, img1, input_m, target  #last ,img1, 
        else:
            return  img, img1,input_m,target,self.data[item]
    def __len__(self):
...
# if __name__ == '__main__':
 ...
 #root='E:/360_VRvideo'
 # for i, (img1_batch,img2_batch,input_m_batch,input_m2_batch,target_batch) in tqdm(enumerate(data_loader), desc='batch', total=len(data_loader)):#last_batch ,img2_batch , target_batch
          
    # viz.images( minmaxscaler(input_m_batch.cpu().numpy()), win='gt-in1')    
    # viz.images( minmaxscaler(input_m2_batch.cpu().numpy()), win='gt-in2')    
