...
def get_kernel_area(ker_loc=(0, 0), theta_c=np.pi / 12, theta=0):
    ...
    """
    #辅助函数用于在球体上旋转后识别内核区域。
    #假设圆心：$ \ mathbf{c} =（x_c，y_c，z_c）$，半径度：$ \ theta_c $。然后，指向$（\ theta，\ phi）$
    #等角投影的圆在i.f.f.
    #$$ x_c \ sin \ theta \ cos \ phi + y_c \ sin \ theta \ sin \ phi + z_c \ cos \ theta = \ cos {\ theta_c} $$
    #我们可以将上面的等式重写为 $$ \ sin（\ phi + \ psi）= C $$
    #其中$ a = x_c \ sin \ theta $，$ b = y_c \ sin \ theta $，$ c = z_c \ cos \ theta $，$ d = \ cos {\ theta_c} $，
    #$ \ sin \ psi = \ frac {a} {\ sqrt {a ^ 2 + b ^ 2}} $，$ \ cos \ psi = \ frac {b} {\ sqrt {a ^ 2 + b ^ 2}} $和$ C = \ frac {dc} {\ sqrt {a ^ 2 + b ^ 2}} $
    #给定内核位置和theta，输出表示有效phi范围的元组列表
    #：param theta_c：内核角半径（rad）
    #：param theta：当前$ \ theta $在球上的位置以标识$ \ phi $范围
    #：return：元组列表（$ \ phi_ {st} $，$ \ phi_ {ed} $）
...
def gen_kernel_grid(ker_shape, ker_loc_theta, ker_radius, target_theta_sr, target_phi_sr, ker_loc_phi=None):
    ...
    """
    #给定冠状核以北极为中心，当前核的位置以及theta和phi的采样率，
    #生成带有原始内核区域的采样网格。
    #：param ker_shape：（$ \ theta $，$ \ phi $）的元组
    #：param ker_loc_theta：当前内核中心位置$ \ theta $
    #：参数ker_radius：
    #：param target_theta_sr：
    #：param target_phi_sr：
    #：return：重新采样网格
