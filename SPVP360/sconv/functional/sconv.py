...
#import torch._thnn1 as thnn1
from torch.autograd import Function     #autograd提供了所有张量操作的自动求微分功能.
from torch.autograd.function import once_differentiable   #实现自动求梯度还有一个很重要的类就是autograd.Function.
import torch.nn.functional as F
from .common import gen_kernel_grid
#from .vision import GridSampler



def kernel_resampling_forward(kernel, grid): #kernel 核函数   grid  梯度
    grid_sz = grid.size()
    backend = thnn.type2backend[kernel.type()]
    assert kernel.dim() == 4 and grid.dim() == 3, 'invalid kernel/grid dimension' #无效的内核/网格维度 内核的维度卷积核的维度
    grid = grid.unsqueeze(0).expand(kernel.size(0), *grid.size()).contiguous()#unsqueeze(0)在第一位增加一个维度  
    #expand参数类型任意，可以往已有列表中添加元素，若添加的是列表，就该列表被当成一个元素存在原列表中，只使list长度增加1.    
    #contiguous 把tensor变成在内存中连续分布的形式
    output = kernel.new(grid_sz[0], kernel.size(1), grid_sz[1], grid_sz[2])
    #GridSampler(backend.library_state, kernel, grid, output,0) 
    backend.SpatialGridSamplerBilinear_updateOutput(backend.library_state, kernel, grid, output,0)   #SpatialUpSamplingBilinear   
    #错误原因：字典中没有SpatialGridSamplerBilinear_updateOutput  
    return output
...
def kernel_resampling_backward(kernel, grid, grad_output):
   ...
   #SpatialGridSamplerBilinear_updateGradInput    SpatialUpSamplingBilinear_updateGradInput
    return grad_kernel, None


def conv2d_updateOutput(image, kernel, stride, padding, output, bias=None):   # #公开PyTorch转换前向功能
    """
    Expose PyTorch conv2d forward function
    """
    _backend = thnn.type2backend[type(image)]    #type() 函数如果你只有第一个参数则返回对象的类型，三个参数返回新的类型对象。
    finput = image.new()
...
class SphericalConv(Function):
    ...
    def _conv_line_forward(
            image, kernel, kernel_grid, bias=None, stride=(), padding='valid', padding_mode='sphere', area=None
    ):
        # resampling kernel
    ...
    def _conv_line_backward(
            grad_out, image, kernel, kernel_grid, finput, bias=None, stride=(), padding='valid', padding_mode='sphere', area=None,
    ):
        # resampling kernel
        kernel_resampled = kernel_resampling_forward(kernel, kernel_grid)
    ...
    def forward(ctx, image, kernel, bias=None, stride=(), padding='valid', padding_mode='sphere', kernel_rad=np.pi/3):
        ...
        if padding == 'valid':
            for theta_idx_ct_in in range(0, image.size(2), stride[1]):
                ...
                kernel_grid, ker_area = gen_kernel_grid(kernel.shape, ker_loc_theta, kernel_rad, target_theta_sr, target_phi_sr)
                ...
                if stride == (1, 1):          
                  ...
                  pad[0], pad[1], pad[2], pad[3] = max(pad[0], padl), max(pad[1], padr), max(pad[2], padt), max(
                    pad[3], padb)
                if stride == (2, 2):          
                   padh, padw = int(kernel_grid.size(0) - 2), int(kernel_grid.size(1) - 2)
                   assert padh >= 0 and padw >= 0
                   padl, padr, padt, padb = int(padw / 2 + 0.5), int(padw / 2), int(padh / 2 + 0.5), int(padh / 2)
                   pad[0], pad[1], pad[2], pad[3] = max(pad[0], padl), max(pad[1], padr), max(pad[2], padt), max(
                    pad[3], padb)    
